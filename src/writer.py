...
- SETUP / HARDWARE / INFRASTRUCTURE is a personal claim too — the most tempting one to fake, so be strict. Do NOT invent what YOU run or how you run it. Never say "I self-host", "I run X on my own hardware", "on my own GPU", "no API keys", "at N tokens per second", or name a specific model/rig you personally run, UNLESS it is in the material above. WRONG: "I run Nemotron Ultra 253B on my own hardware at 50K tokens per second. Self-hosted. No API keys." That is fabricated and a recruiter can trivially disprove it. RIGHT, as general knowledge/opinion (no "I"): "you can self-host open models like Nemotron and skip the API entirely" — a true capability, not a fake personal setup. Do not attach "I" or "my" to any infra/hardware/throughput claim that was not given to you.
...
def build_user_prompt(
    ...
    if topic_key == "my_agent_build":
        prompt_parts.append(
            "\nThis is a BUILD IN PUBLIC post about ONE real thing Lubo shipped this week (from his git log).\n\n"
            "WHAT THIS IS: a SENIOR ENGINEER sharing a real build DECISION and what he learned — NOT a "
            "changelog and NOT a metrics report. A recruiter reading it should think 'this person has "
            "judgment and taste', never 'this person counts lines of code'. The value is the THINKING: the "
            "problem, the call you made, the trade-off, the lesson.\n\n"
            "RULES:\n"
            "- Write about the ONE feature under 'THIS WEEK I BUILT'. LEAD with the problem, the decision, "
            "or the insight — NEVER with numbers or a summary of what changed.\n"
            "- CRITICAL: the card shown with this post ALREADY displays the +lines/-lines and file count. So "
            "do NOT put those numbers in the text. Do NOT open with '617 lines added, 593 deleted'. If scope "
            "matters, say it in plain words ('a heavy refactor', 'ripped out a big chunk', 'one focused "
            "file') — never the raw counts. Counting metrics in the text is exactly what makes these posts "
            "sound hollow. Avoid it.\n"
            "- Tell the real story: what was breaking or annoying, what you decided to do and WHY, what you "
            "learned or what surprised you. One honest, specific lesson beats any stat.\n"
            "- Always say 'I', never 'we' or 'us' — Lubo is solo.\n"
            "- Sound like a senior dev talking through a real call, not a status update. No changelog tone.\n"
            "- NO cliches ('blood and sweat', 'one place to rule them all', 'because AI is hard').\n"
            "- Blank lines between short paragraphs. Hook (first 2 lines) stands alone. Close with a genuine "
            "question about the technical decision or trade-off.\n\n"
            "ANTI-HALLUCINATION (critical):\n"
            "- Use ONLY what the commit message actually says. Do NOT invent performance numbers (ms, latency, "
            "speed, percentages, x-faster), dollar amounts, or user counts. Do NOT fabricate what the code "
            "does beyond the commit message.\n"
            "- If you dont know a detail, keep it vague in words ('it was slow') — never a made-up number.\n"
            "- The line/file counts belong on the CARD, not in the text — do not restate them there."
        )

    # Building-in-public weekly stats — grounded in real WakaTime numbers
    if topic_key == "building_in_public":
        prompt_parts.append(
            "\nThis is a BUILDING IN PUBLIC post about Lubos real coding week. The WakaTime stats below "
            "are CONTEXT for you, not the post.\n\n"
            "WHAT THIS IS: a builder REFLECTING on the shape of their week and what it is like to build "
            "with AI coding agents — NOT a stats report. A reader should walk away with an insight or a "
            "feeling, not a dashboard. The stat CARD shown with this post ALREADY displays the hours, "
            "languages, and cost.\n\n"
            "RULES:\n"
            "- LEAD with an insight or an honest reflection about the week — what dominated it, what "
            "surprised you, the rhythm of it, a realization about coding with AI. NEVER open with a number.\n"
            "- CRITICAL: the hours, token count, test count, per-project time, languages, and cost are ALL "
            "on the CARD. Do NOT restate ANY of them in the text — not as digits AND not spelled out as "
            "words. Writing 'ninety nine hours' or 'fifty three million tokens' or 'nine hundred six tests' "
            "is STILL a stat dump and is banned. Describe the week qualitatively instead ('a heavy week', "
            "'agents carried the boilerplate', 'barely stopped'). Zero stats in the text is the target; one, "
            "in words, only if it is truly the whole point of a line.\n"
            "- Be honest about the SHAPE of the week: a heavy week vs a light one, a week of debugging vs "
            "building, a week the AI carried vs one you fought it. Say it straight.\n"
            ...
            "- Blank lines between short paragraphs. Close with a genuine reflection or question.\n\n"
            "ANTI-HALLUCINATION:\n"
            "- If you do use a number, it must be one from the WakaTime summary above, exact. Invent NO "
            "metric, percentage, comparison, or cost that is not in the data."
        )
