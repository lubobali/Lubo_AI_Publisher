...
- **15c-7 DONE**: all 11 books ingested into app DB `publisher` (4441 chunks), cross-book retrieval verified. NUL-strip fix + TWO-DB test isolation (publisher vs publisher_test) so the suite never wipes the KB (proven). 546 tests.
- Next: 15c-8 (Langfuse + voice-first E2E) — BUT needs the 2.9 post-processor/model fix first (49B output is markdown-heavy + leaks rules). Real grounded E2E only meaningful once output is clean.
