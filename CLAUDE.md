...
- **PHASE 3 DEPLOYED & LIVE (Jun 16)** — dashboard public at **https://publisher.lubot.ai** (SSL, http->https redirect, certbot auto-renew). NOTE: domain is publisher.lubot.ai, NOT lubobali.com (A record was added on lubot.ai; isolated subdomain, prod lubot.ai untouched).
  ...
  - ⚠️ REMAINING: (1) worker container has NO ssh client -> Building-in-Public + My Agent Build fail in prod (5/7 categories work). Fix: mount /srv/lubot-staging + read locally. (2) FIRST REAL POST not done yet (deliberate: generate -> approve in dashboard -> worker publishes).
