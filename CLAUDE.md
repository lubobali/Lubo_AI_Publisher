...
- ⚠️ **REMAINING 2.9 BLOCKER (truth)**: 49B HALLUCINATES solution specifics — a grounded test post invented "23,000 OHLCV records", "Delta Lake on Databricks", "Z-ORDER/VACUUM" that were NOT in the source (source only said "Postgres table got huge"). Violates "always tell the truth". Needs anti-fabrication prompt tuning (strengthen global no-invented-facts rule for the 49B) and/or move to 120B Super when quota frees. This is the real gap before posts are postable.
- Next: anti-hallucination prompt tuning, then 15c-8 voice-first E2E.
