...
15c-6. Writer wiring — inject 2-3 concepts for tech_talk/my_agent_git/ai_news, guardrail block (no book name, no false experience, may ignore), non-fatal KB failures ✅. RAG guardrails LIVE-confirmed: real grounded post did NOT name the book, used the concept as future research (no false "I did it" claim), kept ESL voice.
  WRITER MODEL: 253B retired (~May 6) -> removed from prod. NVIDIA_MODEL now os.getenv("NVIDIA_LLM_MODEL", "nvidia/llama-3.3-nemotron-super-49b-v1") (works). Preferred nemotron-3-super-120b-a12b is 429/quota-limited on this account right now; ultra-500b 404s. Swap via env when quota frees.
  ⚠️ NEW QUALITY ISSUE (Phase 2.9): 49B output is very markdown-heavy and LEAKED a prompt rule literally ("[NO NUMBER - AS PER RULES...]") + emitted "**LinkedIn Post**" headers + raw hashtag JSON. post_processor (tuned for 253B) does not strip markdown/rule-leaks. Posts not presentable until 2.9 tunes post_processor for the new model (or we move to 120B).
15c-7. Ingest script (scripts/ingest_books.py) + ingest_book()/_book_meta() ✅. ALL 11 BOOKS INGESTED into app DB `publisher`: 4441 chunks (AOSA v1 463, v2 394, DE-cookbook 63, DDIA 486, Dive-into-DL 838, LLM-Foundations 760, Fund-of-DE 245, Metadata 173, Python-Harrison 129, ML-Yearning 65, Jurafsky-SLP3 825). Cross-book retrieval verified (sharding->AOSA, attention->Jurafsky, eval->LLM-Foundations, all 0.47-0.53). Raw PDFs kept.
  + clean_text now strips NUL/control chars (Postgres TEXT rejects NUL — caught LLM-Foundations/Jurafsky/Dive-DL on real ingest).
  + TWO-DATABASE design (critical): app DB `publisher` persists KB+posts; tests use disposable `publisher_test` (conftest auto-creates it, all 8 test files default to it). PROVEN: full suite no longer wipes the KB (11 books survive). Real ingest = run scripts/ingest_books.py against `publisher` (in-container DATABASE_URL=publisher-db, or DATABASE_URL=...5433/publisher locally).
...
- **541 tests**, all green, lint clean
- **Phase 2.8 RAG in progress** — 15c-1..15c-6 DONE. Full extract/chunk/embed/store/retrieve chain + writer guardrail wiring (scheduler grounds tech_talk/my_agent_git/ai_news only; book never named; KB failure non-fatal). numpy==2.4.6. Test default topic re-pinned to ai_gadgets (non-grounded) so generic tests skip RAG.
- **WRITER MODEL FIXED**: retired 253B removed from prod. NVIDIA_MODEL = env NVIDIA_LLM_MODEL, default nvidia/llama-3.3-nemotron-super-49b-v1 (live-works). Real generation confirmed + RAG guardrails hold (book unnamed, concept as background, voice intact).
- ⚠️ **2.9 QUALITY**: 49B output is markdown-heavy + leaked a prompt rule into the post; post_processor needs tuning for the new model (or move to 120B Super once its quota frees). Posts not presentable until then.
- **15c-7 DONE**: all 11 books ingested into app DB `publisher` (4441 chunks), cross-book retrieval verified. NUL-strip fix + TWO-DB test isolation (publisher vs publisher_test) so the suite never wipes the KB (proven).
- **2.9 progress**: (a) post_processor now strips markdown (**bold**, ##, ====, * bullets) + model-meta (rule-leaks, 'LinkedIn Post', 'Screenshot URL:', 'Hashtags:') for the 49B — real post renders clean. (b) Langfuse made optional, OFF by default (LANGFUSE_ENABLED, no free-tier cost; tests set it true). 555 tests.
- **MODEL: Nemotron 3 Ultra 550B** (nvidia/nemotron-3-ultra-550b-a55b) — free on NVIDIA NIM, default now. Far better than 49B: clean, on-voice, low-hallucination. 49B = env fallback.
- **15c-8 quality pass (mostly done)**: hardened TRUTH in system prompt (no invented numbers->words; no invented tools/companies; personal-story-vs-opinion rule; explicit no-markdown). brand normalize (luBot->LuBot). Real E2E verified on 550B across tech_talk/ai_news/biohacker — ALL truthful, senior, on-voice (tech_talk: "Not there yet. Still experimenting"; ai_news cites LuBot's REAL cascade arch; biohacker: real science, no fabricated experiments). 561 tests.
- Paid-API research (if NVIDIA free tier 429s LuBot prod at scale): OpenRouter (breadth, ~$0.09/$0.45 per M for 120B) or DeepInfra (cheaper, prompt caching). Publisher itself = pennies/free.
- **15c-8 DONE — all categories verified truthful + senior + on-voice on the 550B**: tech_talk (frames as opinion, "still experimenting"), ai_news (cites real LuBot cascade arch), biohacker (NOW OWNS the identity — "morning light is the most underrated biohack", no disclaimer), building_in_public (exact real WakaTime numbers, no stat leaks), my_agent_build (exact real git +lines/files). Truth leaks closed: no invented numbers/tools/stories; no training-knowledge stat leaks (was injecting wrong "17 workers"); brand+lobot typo normalized. 562 tests.
- **Phase 2.8 (RAG) + 2.9 (quality) essentially COMPLETE.** Content engine produces $300k-caliber posts.
- **7-DAY E2E DRY RUN DONE** (scripts/test_pipeline.py vs app DB). 5/7 ran full pipeline (scrape->dedup->KB->550B->screenshot->pending) with real screenshots; 2/7 (Building in Public, My Agent Build) only fail from bare shell because they SSH to staging — works IN the container (Git Insights LIVE since 2.7). The dry run exposed 3 real TRUTH bugs, all FIXED:
  1. Tech Talk invented a project ('built a bitcoin pipeline this week' off a physics podcast) -> reframed Tech Talk to senior OPINION/expertise, never a claimed personal project.
  2. My Agent stated false product facts ('GPT-4/Claude/Gemini', '122K/36 tables/42 endpoints') -> the my_agent_features LIST was stale/wrong; rewrote it accurate (100% NVIDIA Nemotron, 112K/34/28, the Cascade, 3 modes, stock advisor). Prompt: exact numbers, no fake scenarios.
  3. Training-knowledge stat leaks ('17 nightly workers') -> TRUTH rule covers facts the model thinks it knows.
- **ALL 7 categories verified truthful + senior + on-voice on 550B.** Content engine produces 7 perfect posts/week. 563 tests.
- **PHASE 3 DEPLOYED & LIVE (Jun 16)** — dashboard public at **https://publisher.lubot.ai** (SSL, http->https redirect, certbot auto-renew). NOTE: domain is publisher.lubot.ai, NOT lubobali.com (A record was added on lubot.ai; isolated subdomain, prod lubot.ai untouched).
  - Stack (Hetzner, docker compose): publisher-web (uvicorn dashboard+API on 127.0.0.1:8100, single-page static/dashboard.html) + publisher-worker (src/cron.py APScheduler: daily_generation + publish_approved every 5min) + publisher-db (persistent KB). nginx vhost /etc/nginx/sites-enabled/publisher.lubot.ai.
  - Dockerfile fix: chromium runtime libs by Debian name + 'playwright install chromium' (NOT --with-deps). .dockerignore keeps .venv/books/.git out.
  - LinkedIn OAuth DONE: token + person URN (urn:li:person:02U_1s-eJB) in .env. Needed the "Sign In with LinkedIn using OpenID Connect" product for the profile scope; token via OAuth2 tools Create-token with openid+profile+email+w_member_social.
  - ✅ RESOLVED (Jun 16): (1) ALL 7/7 categories now run in prod. Worker reads staging git + WakaTime LOCALLY (STAGING_LOCAL=1 + /srv/lubot-staging:ro mount + git in image) instead of SSH — git_insights/wakatime_insights gained a local-read mode; SSH stays default for tests/Mac. Verified in-container: My Agent Build (#11, real +1905/-1457 git numbers, git-commit.png) + Building in Public (#12, real 65h/129% WakaTime, wakatime-week.png). (2) FIRST REAL POST is LIVE — biohacking post published to LinkedIn (urn:li:share:7472521130285793280) via the full approve-on-phone → worker-auto-publishes loop. LinkedIn API version bumped 202401->202506 (env-overridable) to fix a 426.
  - Runner capacity bumped 1->2 (/opt/forgejo-runner/config.yaml) so CI does not queue.
